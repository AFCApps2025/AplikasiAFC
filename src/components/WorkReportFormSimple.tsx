...
const WorkReportForm = () => {
  ...
  const [unitPhotos, setUnitPhotos] = useState<{
    [key: number]: { files: File[]; previews: string[] };
  }>({});
      ...
      const fetchBookingData = async () => {
        try {
          ...
          if (bookingData) {
            ...
            const { data: existingReport, error: reportError } = await (
              supabase as any
            )
              .from("work_reports")
              .select("*")
            ...
            if (bookingData.status === "komplain" && existingReport) {
              console.log(
                "📋 Auto-filling form with previous work report data for komplain"
              );

              // Parse existing data
              const existingUnits = existingReport.units
                ? JSON.parse(existingReport.units)
                : [];
              const existingWorkTypes = existingReport.jenis_pekerjaan
                ? JSON.parse(existingReport.jenis_pekerjaan)
                : [];
              const existingPhotos = existingReport.foto_url
                ? JSON.parse(existingReport.foto_url)
                : [];

              setFormData((prev) => ({
                ...prev,
                namaPelanggan:
                  existingReport.nama_pelanggan || bookingData.nama || "",
                alamatPelanggan:
                  existingReport.alamat_pelanggan || bookingData.alamat || "",
                noWaPelanggan:
                  existingReport.no_wa_pelanggan || bookingData.no_hp || "",
                bookingUnitCount:
                  existingUnits.length || bookingData.jumlah_unit || 1,
                units:
              ...
              const initialWorkTypes = Array.from(
                { length: unitCount },
                () => ({
                  jenisPekerjaan: bookingData.jenis_layanan || "",
                })
        ...
        try {
          ...
          if (rejectedReport) {
            ...
            setFormData((prev) => ({
              ...prev,
              namaPelanggan:
                rejectedReport.nama_pelanggan || customerName || "",
              alamatPelanggan:
                rejectedReport.alamat_pelanggan || customerAddress || "",
              noWaPelanggan:
                rejectedReport.no_wa_pelanggan || customerPhone || "",
        ...
        const fetchBookingDetails = async () => {
          if (bookingData.booking_id) {
            try {
              ...
              const initialWorkTypes = Array.from(
                { length: unitCount },
                () => ({ jenisPekerjaan: bookingData.jenis_pekerjaan || "" })
              );
  ...
  const addUnit = () => {
    ...
    if (
      ...
      formData.units.length + 1 > formData.bookingUnitCount
    ) {
      toast({
        title: "Info",
        description: `Unit ditambahkan. Total unit sekarang: ${
          formData.units.length + 1
        } (booking: ${formData.bookingUnitCount} unit)`,
      });
    }
  };
  ...
  const removeUnit = (index: number) => {
    ...
    if (formData.units.length > 1) {
      ...
      if (
        ...
        toast({
          title: "Info",
          description: `Unit dihapus. Total unit sekarang: ${
            formData.units.length - 1
          } (booking: ${formData.bookingUnitCount} unit)`,
        });
      }
  ...
  const uploadPhotos = async (photos: File[]): Promise<string[]> => {
    ...
    for (const photo of photos) {
      try {
        ...
        const {
          data: { publicUrl },
        } = supabase.storage.from("work-report-photos").getPublicUrl(filePath);
    ...
    try {
      ...
      if (findData) {
        ...
        if (updateError) {
          ...
          console.log(
            "Verification - booking status after update:",
            verifyData
          );
  ...
  const sendAffiliateNotification = async (
    ...
    try {
      ...
      if (jenisLayananLower.includes("cek") || statusLower === "komplain") {
        console.log(
          'Skipping affiliate notification - jenis_layanan is "cek" or status is "komplain"'
        );
...
*FROST*`;
      ...
      if (reportData.booking_id) {
        try {
          ...
          if (!bookingError && bookingData?.kode_referral) {
            console.log(
              "Found referral code for work report:",
              bookingData.kode_referral
            );
            ...
            if (!partnerError && partnerData?.nomor_whatsapp) {
              ...
              console.log(
                "Added referral contact to recipients:",
                partnerPhone
              );
            }
          }
      ...
      const sendPromises = validRecipients.map(async (phoneNumber) => {
        try {
          ...
          console.log(
            `WhatsApp to ${formattedPhone}: ${result ? "SUCCESS" : "FAILED"}`
          );
          return result;
        } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      formData.bookingUnitCount > 0 &&
      formData.units.length !== formData.bookingUnitCount
    ) {
      console.log(
        `INFO: Unit count berbeda - Booking: ${formData.bookingUnitCount}, Laporan: ${formData.units.length}`
      );
    ...
    try {
        ...
        if (unitPhotoData.files.length > 0) {
          try {
            ...
            const urls = await Promise.race([
              ...
              new Promise<string[]>((_, reject) =>
                setTimeout(
                  () => reject(new Error("Unit photo upload timeout")),
                  isMobile ? 30000 : 20000
                )
              ),
            ]);
            ...
            console.warn(
              `Unit ${unitIndex} photo upload failed, continuing without photos:`,
              error
            );
      ...
      const isComplainUpdate =
        sessionStorage.getItem("isComplainUpdate") === "true";

      if (isComplainUpdate && existingReportId) {
        console.log(
          "🔄 Updating existing work report for komplain:",
          existingReportId
        );
        ...
        const allPhotosForThisUnit = [
          ...generalPhotoUrls,
          ...unitPhotosForThisUnit,
        ];
        ...
        const updateData = {
          ...
          tanggal_dikerjakan:
            formatDateForDatabase(formData.tanggalDikerjakan) ||
            new Date().toISOString().split("T")[0],
          foto_url:
            allPhotosForThisUnit.length > 0
              ? JSON.stringify(allPhotosForThisUnit)
              : null,
        ...
        const reportPromises = formData.units.map(async (unit, index) => {
          ...
          const reportData = {
            ...
            no_wa_pelanggan: formData.noWaPelanggan || "0",
            jenis_pekerjaan:
              formData.workTypes[index]?.jenisPekerjaan ||
              formData.workTypes[0]?.jenisPekerjaan ||
              "Service",
            teknisi: formData.teknisi || "Unknown Technician",
      ...
      if (hasErrors) {
        ...
        const existingReports = JSON.parse(
          localStorage.getItem("workReports") || "[]"
        );
        ...
        formData.units.forEach((unit, index) => {
          const reportData = {
            ...
            tanggal_dikerjakan: formatDateForDatabase(
              formData.tanggalDikerjakan
            ),
            jenis_pekerjaan:
              formData.workTypes[index]?.jenisPekerjaan ||
              formData.workTypes[0]?.jenisPekerjaan,
        ...
        toast({
          title: "Peringatan",
          description:
            "Laporan disimpan offline. Akan dikirim otomatis saat koneksi stabil.",
          variant: "destructive",
        });
      } else {
      ...
      if (formData.bookingId && formData.kodeReferral) {
        ...
        console.log(
          "Status: Laporan submitted, referral akan di-update saat approved"
        );
      }

      // All roles now go through approval workflow - no direct WhatsApp sending
      console.log(
        "Work report saved, waiting for approval before sending WhatsApp"
      );
      toast({
        title: "Laporan Dikirim",
        description:
          "Laporan kerja berhasil dikirim dan menunggu persetujuan admin/manager",
      });
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleInputChange("namaPelanggan", e.target.value)
                }
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  handleInputChange("alamatPelanggan", e.target.value)
                }
            ...
            <div>
              ...
              <input
                ...
                value={
                  currentUser?.role === "teknisi"
                    ? formData.noWaPelanggan.replace(/./g, "\u2022")
                    : formData.noWaPelanggan
                }
                onChange={(e) =>
                  handleInputChange("noWaPelanggan", e.target.value)
                }
                placeholder={
                  currentUser?.role === "teknisi"
                    ? "\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022"
                ...
                className={`w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                  (isFromBooking && formData.noWaPelanggan) ||
                  currentUser?.role === "teknisi"
              ...
              {currentUser?.role === "teknisi" && (
                <p className="text-xs text-gray-500 mt-1">
                  Nomor telepon disembunyikan untuk akun teknisi
                </p>
              )}
            </div>
          </div>
              ...
              <div>
                ...
                {formData.bookingUnitCount > 0 && (
                  <p className="text-sm text-blue-600 mt-1">
                    Booking memerlukan {formData.bookingUnitCount} unit (saat
                    ini: {formData.units.length} unit)
                  </p>
                )}
              </div>
              <button
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    value={unit.noUnit}
                    onChange={(e) =>
                      handleUnitChange(index, "noUnit", e.target.value)
                    }
                  ...
                  {unit.merk === "MERK LAIN" && (
                    <div className="mt-2">
                      ...
                      <input
                        ...
                        value={unit.merkLain}
                        onChange={(e) =>
                          handleUnitChange(index, "merkLain", e.target.value)
                        }
                ...
                <div>
                  ...
                  <input
                    ...
                    value={unit.spekUnit}
                    onChange={(e) =>
                      handleUnitChange(index, "spekUnit", e.target.value)
                    }
                  ...
                  {unitPhotos[index]?.files.length > 0 && (
                    <div className="mt-3">
                      <p className="text-sm text-blue-600 mb-2">
                        {unitPhotos[index].files.length} foto dipilih untuk unit{" "}
                        {index + 1}
                      </p>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        {unitPhotos[index].previews.map(
                          (preview, photoIndex) => (
                            <div key={photoIndex} className="relative">
                              <img
                                src={preview}
                                alt={`Unit ${index + 1} Preview ${
                                  photoIndex + 1
                                }`}
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  removeUnitPhoto(index, photoIndex)
                                }
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleInputChange("tanggalDikerjakan", e.target.value)
                }
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                required
              />
              ...
              {formData.workTypes.map((workType, index) => (
                ...
                >
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleWorkTypeChange(index, e.target.value)
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    required
                  >
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2">
                      Keterangan Unit {index + 1} (Kondisi Unit)
                    </label>
                    <textarea
                      value={formData.units[index]?.keterangan || ""}
                      onChange={(e) =>
                        handleUnitChange(index, "keterangan", e.target.value)
                      }
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2">
                      Catatan Internal Unit {index + 1}
                    </label>
                    <textarea
                      value={formData.units[index]?.internalNotes || ""}
                      onChange={(e) =>
                        handleUnitChange(index, "internalNotes", e.target.value)
                      }
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-blue-500 mt-1">
                        Partner akan mendapat notifikasi +1 poin setelah
                        pekerjaan selesai
                      </p>
                    </div>
                  </div>
